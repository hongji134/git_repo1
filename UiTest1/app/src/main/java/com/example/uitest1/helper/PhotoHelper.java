...
public class PhotoHelper {
    ...
    private PhotoHelper() {}        // 생성자가 private -> 외부에서는 new로 객체 생성을 할 수 없다.
    // 싱글톤 끝
     ...
     */
     public String getNewPhotoPath() {
         ...
         File dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM);
                            //getExternalStoragePublicDirectory() : 공용폴더 얻어오기
         // 폴더가 없으면 만들기
         if(!dir.exists()) {    // 공용 폴더가 있는지 없는지 검사
             dir.mkdirs();      // 중간에 경로가 없어도 만들어버림.
         }
         Log.d("[INFO] dir = ", "dir = " + dir);        // 출력

         String photoPath = dir.getAbsolutePath() + "/" + fileName;
         Log.d("[INFO] dir = ", "photoPath = " + photoPath);
         return photoPath;
     }
     ...
     */
     public Bitmap getThumb(Activity activity, String path) {
         ...
         if(fscale > maxScale) {
            ...
            // ex) 이미지의 긴 축이 2400px, 스마트폰의 긴 축이 800px 일 때
            //      2400 / 800 = 3  => 샘플링 사이즈 = 3
            int sampleSize = fscale / maxScale;
            // 사이즈 불러오기 직전 새 비트맵 옵션 생성
             BitmapFactory.Options options2 = new BitmapFactory.Options();
             // 샘플 사이즈 설정 --> 3으로 지정하면, 1/3 크기가 된다.
             options2.inSampleSize = sampleSize;
             // 이미지 읽어오기 - sampleSize만큼 크기를 줄여서 읽어온다.
             bmp = BitmapFactory.decodeFile(path, options2);        // options2가 없으면 원본 크기 그대로 가져온다.
                                                                    // options2가 있으면 크기를 줄여서 가져온다.
         } else {
    ...
    public Bitmap rotate(Bitmap bitmap, int degrees) {
         if(degrees != 0 && bitmap != null) {
             ...
             Matrix m = new Matrix();       // 값을 세팅하는 클래스
             // public void setRotate(float degrees, float px, float py) : bitmap 기준으로 위치와 각도. 각도 설정
             // bitmap(가운데) 기준으로 얼만큼 돌려라.
             ...
             if(bitmap != converted) {
                 bitmap.recycle();          // 같지 않다면 기존에 있는 bitmap (메모리에서)삭제
                 bitmap = converted;
             }
         }
         return bitmap;
